  ...
  function verifyProof(bytes calldata proof, uint256[] calldata instances) public returns (bool) {
    assembly {
...

      {
        ...
        // Read evaluations
        for { let proof_cptr_end := add(proof_cptr, 0x08e0) } lt(proof_cptr, proof_cptr_end) {} {
      ...
      // Compute lagrange evaluations and instance evaluation
      {
        ...
        for { let l_i_cptr_end := add(X_N_MPTR, 0xc0) } lt(l_i_cptr, l_i_cptr_end) { l_i_cptr := add(l_i_cptr, 0x20) } {
      ...
      // Compute quotient evavluation
      {
        ...
        }
        {
          ...
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x08e4), mulmod(beta, calldataload(0x0c64), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0904), mulmod(beta, calldataload(0x0c84), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0924), mulmod(beta, calldataload(0x0ca4), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0944), mulmod(beta, calldataload(0x0cc4), R), R), gamma, R), R)
          ...
          let eval :=
            addmod(
              left_sub_right, sub(R, mulmod(left_sub_right, addmod(mload(L_LAST_MPTR), mload(L_BLIND_MPTR), R), R)), R
            )
          quotient_eval_numer := addmod(mulmod(quotient_eval_numer, y, r), eval, r)
        }
        {
          ...
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0964), mulmod(beta, calldataload(0x0ce4), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0984), mulmod(beta, calldataload(0x0d04), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x09a4), mulmod(beta, calldataload(0x0d24), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x09c4), mulmod(beta, calldataload(0x0d44), R), R), gamma, R), R)
        ...
        }
        {
          ...
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x09e4), mulmod(beta, calldataload(0x0d64), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(calldataload(0x0a24), mulmod(beta, calldataload(0x0d84), R), R), gamma, R), R)
          lhs :=
            mulmod(lhs, addmod(addmod(mload(INSTANCE_EVAL_MPTR), mulmod(beta, calldataload(0x0da4), R), R), gamma, R), R)
        ...
        }
        {
          ...
          let eval :=
            mulmod(
              addmod(1, sub(R, addmod(mload(L_BLIND_MPTR), mload(L_LAST_MPTR), R)), R), addmod(lhs, sub(R, rhs), R), R
            )
          quotient_eval_numer := addmod(mulmod(quotient_eval_numer, y, r), eval, r)
        }
        {
      ...
      // Compute pairing lhs and rhs
      {
        ...
        }
        {
          ...
          for {
            let mptr := 0x0440
            let mptr_end := 0x04a0
          } lt(mptr, mptr_end) { mptr := add(mptr, 0x20) } { mstore(mptr, mulmod(mload(mptr), diff_0_inv, R)) }
        }
        {
